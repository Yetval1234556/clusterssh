...
def do_test(cfg, model, iteration, val_loader=None, best_val_loss=float("inf")):
    new_state_dict = model.teacher.state_dict()
    val_loss = None

    if val_loader is not None:
        logger.info(f"Evaluating validation set at iteration {iteration}")
        model.eval()
        total_val_loss = 0.0
        num_batches = 0
        with torch.no_grad():
            for data in val_loader:
                loss_dict, _ = model.forward_backward(data, teacher_temp=cfg.teacher.teacher_temp, backprop=False)
                # Compute reduced loss similar to train
        ...
        val_loss = total_val_loss / max(1, num_batches)
        logger.info(f"Validation loss at iteration {iteration}: {val_loss:.4f}")
    ...
    if distributed.is_main_process():
        ...
        if val_loss is not None and val_loss < best_val_loss:
            best_dir = os.path.join(cfg.train.output_dir, "best_checkpoint")
            os.makedirs(best_dir, exist_ok=True)
            best_ckp_path = os.path.join(best_dir, "best_teacher.pth")
            torch.save({"teacher": new_state_dict, "val_loss": val_loss, "iteration": iteration}, best_ckp_path)
            logger.info(f"Saved new best checkpoint with val_loss {val_loss:.4f}")

    return val_loss if val_loss is not None else best_val_loss
...
def do_train(cfg, model, resume=False):
    ...
    if getattr(cfg.train, "val_dataset_path", None):
        val_dataset = make_dataset(
            dataset_str=cfg.train.val_dataset_path,
            transform=data_transform,
            target_transform=lambda _: (),
        )
    ...
    for data in metric_logger.log_every(
        ...
        if cfg.evaluation.eval_period_iterations > 0 and (iteration + 1) % cfg.evaluation.eval_period_iterations == 0:
            best_val_loss = do_test(cfg, model, f"training_{iteration}", val_loader=val_data_loader, best_val_loss=best_val_loss)
