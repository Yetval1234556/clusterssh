...
set SCRIPTDIR=%~dp0
set SSH=ssh -o ConnectTimeout=10 -o BatchMode=yes %CLUSTER_USER%@%CLUSTER_HOST%
...
if errorlevel 1 (
    echo.
    echo  ERROR: Cannot reach %CLUSTER_HOST% — check VPN / SSH key / network.
    echo  Tip  : Run  ssh %CLUSTER_USER%@%CLUSTER_HOST%  to debug.
    exit /b 1
)
...
echo.
echo    ┌────────────────────────────────────────────────────────────────────┐
echo    │   Archive inventory BEFORE sync                                    │
echo    ├────────────────────────────────────────────────────────────────────┤
%SSH% "mkdir -p ~/bloomi/'New Data'/extracted && ls ~/bloomi/'New Data'/extracted/ 2>/dev/null | grep -E '^archive[0-9]+' | sort -V | while read a; do echo \"    │   $a\"; done || echo '    │   (none found)'"
echo    └────────────────────────────────────────────────────────────────────┘
echo.
